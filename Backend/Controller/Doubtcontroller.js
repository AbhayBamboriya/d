...
router.post('/',isLoggedIn,async(req, res,next) => {
    ...
    console.log('let',title,description,subject);
    if(!title || !description || !subject){
        return next(new AppError('Title,Description,Subject are Required',400))
    }
    // if (req.user.role !== 'student') {
    //     return res.status(403).send('Only students can post doubts.');
    // }

    await Doubt.PostDoubt(title,description,userId,name,subject);
...
router.post('/upload', isLoggedIn,upload.single('pdf'),async (req, res,next) => {
  ...
  router.post('/uploadBook',isLoggedIn, upload.single('bookFile'), async (req, res,next) => {
    try {
      ...
      res.status(200).json({success:true, message: 'Book uploaded successfully!', data: uploadData });
    } catch (error) {
...
router.post('/uploadPaper',isLoggedIn, upload.single('questionPaper'), async (req, res) => {
    try {
      ...
      res.status(200).json({success:true, message: 'Question paper uploaded successfully!', data: uploadData });
    } catch (error) {
      console.error('Error uploading file:', error);
