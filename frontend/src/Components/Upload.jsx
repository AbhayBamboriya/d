      ...
      console.log('type. ',type);
        ...
        if(type=="Book"){
          const toastId=toast.loading('Wait, File is Uploading') 
            ...
            try {
              console.log('req of uploadvjdjfjdf',formData);
              // ('http://localhost:5000/doubts/upload'
              ...
              if(response?.data?.success) toast.update(toastId, { render: 'Book Uploaded successful!', type: 'success', isLoading: false });
          ...
          const formData = new FormData();
            ...
            try {
              ...
              // if(response)  loading=false;
              if(response?.data?.success) toast.update(toastId, { render: 'Notes Uploaded successful!', type: 'success', isLoading: false });
          ...
          const toastId=toast.loading('Wait, PYQ is Uploading') 
            ...
            if (!Number.isInteger(Number(year))) {
              ...
              toast.error("Year of paper should be integer")
  return ;
}
            const formData = new FormData();
            formData.append('subjectName',subject)
            ...
            try {
              ...
              // if(response)  loading=false;
              if(response?.data?.success) toast.update(toastId, { render: 'PYQ Uploaded successful!', type: 'success', isLoading: false });
