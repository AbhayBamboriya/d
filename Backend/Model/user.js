...
export const Doubt={
  PostDoubt: async (title,description,userId,name,subject) => {
    console.log('test',title,description,name,userId,subject);
    // const name="try";
    return new Promise((resolve, reject) => {
  connection.query(
    'INSERT INTO doubts (title, Subject,description, Name,user_id) VALUES (?,?, ?, ?,?)',
    [title, subject,description,name, Number(userId)],
    (err, results) => {
      if (err) reject(err);
  ...
  UploadBook:async(data)=>{
    // console.log();
    console.log('data is books ', data);
